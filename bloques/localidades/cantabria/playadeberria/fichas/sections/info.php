<?php $li_encabezado = '
    <h1 class="text-center">Playa de Gorliz</h1>
    <p class="text-center">Descubre la Playa de Gorliz, un rincón encantador en la costa de Bizkaia. Con su extensa franja de arena dorada y un entorno natural impresionante, Gorliz ofrece un ambiente relajante ideal para disfrutar del sol, practicar deportes acuáticos y explorar paisajes costeros. Además, su paseo marítimo cuenta con una selección de bares y restaurantes, ofreciendo una experiencia completa para todos los visitantes que buscan tranquilidad y belleza natural.</p>
'; ?>

<?php $li_descripcion_detallada = '
<h2 class="section-header text-center">Información General</h2>
<div class="info-box">
    <ul class="section-content">
        <li>La Playa de Berria, en Cantabria, es un destino ideal para quienes buscan relajación y actividades acuáticas en un entorno natural único.</li>
        <li>Con su amplia franja de arena dorada y excelentes condiciones para el surf, es popular tanto entre surfistas como familias.</li>
        <li>La cercana localidad de Santoña ofrece una excelente oferta gastronómica con pescados y mariscos frescos, así como pintxos típicos de la región.</li>
    </ul>
</div>
'
; ?>

<?php $li_descripcion_detallada2 = '

<h3 class="section-header text-center">Entorno Natural</h3>
<div class="info-box">
    <ul class="section-content">
        <li>La playa está rodeada de dunas y áreas protegidas como las marismas de Santoña, Victoria y Joyel.</li>
        <li>Este entorno único es perfecto para los amantes de la naturaleza y las caminatas al aire libre.</li>
        <li>El área es hogar de diversas especies de fauna y flora.</li>
    </ul>
</div>

<h3 class="section-header text-center">Gastronomía</h3>
<div class="info-box">
    <ul>
        <li>En Santoña, cercano a Berria, se puede disfrutar de una gran oferta de restaurantes.</li>
        <li>Los restaurantes sirven mariscos frescos, pescados y pintxos típicos de Cantabria.</li>
        <li>Esto asegura una experiencia culinaria inigualable tras un día de playa.</li>
    </ul>
</div>
'; ?>


<?php $li_descripcion_detallada3 = '
<h3>Características Principales</h3>
<div class="info-box">
    <ul>
        <li><u><strong>Longitud:</strong></u> 2 kilómetros de arena dorada.</li>
        <li><u><strong>Anchura:</strong></u> Hasta 150 metros en marea baja.</li>
        <li><u><strong>Oleaje:</strong></u> Ideal para el surf, de moderado a fuerte.</li>
        <li><u><strong>Entorno:</strong></u> Flanqueada por los montes Buciero y Brusco.</li>
        <li><u><strong>Accesibilidad:</strong></u> Fácil acceso con aparcamiento cercano.</li>
    </ul>
</div>
'; ?>


<?php $li_mapa_ubicacion = '
    <h2>Cómo Llegar</h2>
    <p>La Playa de Berria es de fácil acceso tanto en coche como en transporte público.</p>
    <ul>
        <li><strong>En coche:</strong> Desde Santoña, la Playa de Berria se encuentra a aproximadamente 10 minutos en coche por la carretera CA-141. Hay varias opciones de aparcamiento cerca de la playa, incluyendo zonas gratuitas.</li>
        <li><strong>En autobús:</strong> La playa está conectada mediante autobuses desde Santoña y otras localidades cercanas. Consulta los horarios y rutas de la empresa de autobuses ALSA.</li>
        <li><strong>A pie o en bicicleta:</strong> Berria y sus alrededores cuentan con caminos escénicos y carriles bici que facilitan el acceso a la playa. Es una opción ideal para disfrutar del paisaje costero y el aire libre.</li>
    </ul>
    <h3>Mapa de la Playa de Berria</h3>
    <iframe
        src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2900.8411948448353!2d-3.4521996!3d43.4607339!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0xd4940245c48bb7d%3A0x403b7baee0b9ab92!2sPlaya%20de%20Berria!5e0!3m2!1ses!2ses!4v1689340574832!5m2!1ses!2ses"
        width="100%"
        height="450"
        style="border:0;"
        allowfullscreen=""
        loading="lazy"
        referrerpolicy="no-referrer-when-downgrade">
    </iframe>
'; ?>

<?php $li_actividades_recomendadas = '
    <h2>Actividades en la Playa de Berria</h2>
    <div class="info-box">
        <ul>
            <li><span class="highlight">Surf:</span> La Playa de Berria es conocida por sus excelentes condiciones para el surf, con olas que atraen tanto a principiantes como a surfistas experimentados. Hay varias escuelas de surf en la zona que ofrecen clases y alquiler de equipos.</li>
            <li><span class="highlight">Kitesurf:</span> La playa también es un lugar popular para el kitesurf, gracias a sus amplios espacios y buenas condiciones de viento.</li>
            <li><span class="highlight">Senderismo:</span> Explora las rutas de senderismo que rodean la playa, incluyendo caminos que ofrecen vistas panorámicas del mar y del paisaje costero.</li>
            <li><span class="highlight">Observación de Aves:</span> Las áreas naturales cercanas a la playa, como las marismas de Santoña, son ideales para la observación de aves, con una gran variedad de especies locales y migratorias.</li>
            <li><span class="highlight">Pesca:</span> La pesca desde la orilla es una actividad popular en Berria, con oportunidades para capturar especies locales en las tranquilas aguas cercanas.</li>
            <li><span class="highlight">Paseos en Bicicleta:</span> Disfruta de paseos en bicicleta por las rutas cercanas a la playa, ideales para explorar el entorno natural y disfrutar del paisaje costero.</li>
        </ul>
    </div>
'; ?>

<?php $li_mas_info = '
    <h2>Más Información</h2>
    <div class="info-box">
        <p>Para más información sobre la Playa de Berria, puedes contactarnos a través de los siguientes medios:</p>
        <ul>
            <li><strong>Teléfono:</strong> +34 942 67 00 00</li>
            <li><strong>Email:</strong> turismo@santona.es</li>
            <li><strong>Oficina de Turismo:</strong> Plaza de la Constitución, 39770 Santoña, Cantabria
                <a href="http://www.turismosantona.es/"><strong>Web</strong></a>
            </li>
        </ul>
    </div>

'; ?>


<?php $li_services = '
<div class="info-box">
    <ul>
        <li><span class="highlight">Socorristas:</span> Servicio de socorrismo disponible durante la temporada alta para garantizar la seguridad de todos los bañistas en la Playa de Noja.</li>
        <li><span class="highlight">Accesibilidad:</span> La playa cuenta con accesos adaptados para personas con movilidad reducida, incluyendo rampas y sillas anfibias.</li>
        <li><span class="highlight">Duchas y baños:</span> Instalaciones de duchas de agua dulce y baños públicos ubicados a lo largo de la Playa de Noja.</li>
        <li><span class="highlight">Aparcamiento:</span> Zona de aparcamiento amplia y gratuita cerca de la playa, con plazas reservadas para personas con discapacidad.</li>
        <li><span class="highlight">Alquiler de sombrillas y hamacas:</span> Servicios de alquiler disponibles durante los meses de verano para mayor comodidad de los visitantes en la Playa de Noja.</li>
        <li><span class="highlight">Chiringuitos:</span> Varios chiringuitos a lo largo de la playa que ofrecen una variedad de refrescos, helados y comidas ligeras.</li>
        <li><span class="highlight">Centro de Deportes Náuticos:</span> Alquiler de equipos para practicar surf, paddle surf y otros deportes acuáticos en la Playa de Noja.</li>
    </ul>
</div>
'; ?>





<?php
$imagenes = [
    [
        'src' => './../img/vista-panoramica-playa-Berria-800x533.jpg',
        'alt' => 'Vista panorámica de la Playa de Berria',
        'figcaption' => 'Vista panorámica de la Playa de Berria, Cantabria. Fuente:',
        'href' => 'https://www.cantabriarural.com/playas/playa-de-berria.html',
        'titulo_href' => 'cantabriarural.com'
    ],
    [
        'src' => './../img/dunas-arena-playa-Berria-y-punta-de-El-Brusco.jpg',
        'alt' => 'Dunas de arena en la Playa de Berria" class="img-fluid',
        'figcaption' => 'Dunas de arena en la Playa de Berria, Cantabria. Fuente:',
        'href' => 'https://elpais.com/elviajero/2021/01/21/actualidad/1611229533_330658.html#',
        'titulo_href' => 'redcantabrarural.com'
    ],
    [
        'src' => './../img/surfistas-en-la-playa-de-Berria.jpg',
        'alt' => 'Surfistas en la Playa de Berria',
        'figcaption' => ' Surfistas en la Playa de Berria, Cantabria. Fuente:',
        'href' => 'https://www.youtube.com/watch?v=2qedUp4dVYM',
        'titulo_href' => 'Youtube'
    ],
    [
        'src' => './../img/atardecer-playa-de-Berria.jpg',
        'alt' => 'Atardecer en la Playa de Berria',
        'figcaption' => 'Familias disfrutando en la Playa de Gorliz, Vizcaya.',
        'href' => 'https://www.tripadvisor.es/LocationPhotoDirectLink-g608999-d4556139-i336517190-Playa_De_Berria-Santona_Cantabria.html',
        'titulo_href' => 'tripadvisor'
    ],
    [
        'src' => './../img/playa-de-berria-santona.jpg',
        'alt' => 'Vista de la Playa de Berria, Santoña',
        'figcaption' => 'Playa de Berria, Santoña. Fuente:',
        'href' => 'https://turismodecantabria.com',
        'titulo_href' => 'Turismo de Cantabria'
    ],
    [
        'src' => './../img/monte-buciero-cercano-playa-de-Berria.jpg',
        'alt' => 'Vistas al Monte Buciero cercano a la playa Berria',
        'figcaption' => 'Vistas al Monte Buciero cercano a la Playa de Berria. Fuente:',
        'href' => 'https://www.tripadvisor.es/LocationPhotoDirectLink-g608999-d4555728-i117278266-Monte_Buciero-Santona_Cantabria.html',
        'titulo_href' => 'Tripadvisor'
    ]
]; ?>