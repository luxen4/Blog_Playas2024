<!-- Descripción detallada -->
<section id="informacion_general" class="container my-5">
    <h2 class="section-header text-center">Información General de la Playa de Laredo</h2>
    <div class="info-box">
        <p class="section-content">La Playa de Laredo, también conocida como "La Salvé", es uno de los destinos más destacados de Cantabria. Con su extensa franja de arena dorada que se extiende por más de 4 kilómetros, es el lugar ideal para disfrutar de un día de sol y mar. La playa ofrece un ambiente relajado y es muy popular entre las familias y los visitantes que buscan una experiencia costera completa.</p>
        <p class="section-content">Además de su magnífica playa, Laredo cuenta con un vibrante paseo marítimo lleno de bares y restaurantes que ofrecen una variedad de opciones gastronómicas. La playa también está cerca de zonas verdes y rutas de senderismo, lo que permite combinar la relajación en la playa con actividades al aire libre. La Playa de Laredo es el destino perfecto para disfrutar de un entorno natural impresionante y una experiencia costera enriquecedora.</p>
    </div>

    <h2>Descripción General</h2>
    <p>
        La Playa de Laredo, conocida como "La Salvé", es una de las playas más extensas de la costa cantábrica, con más de 4 kilómetros de arena dorada y fina. Ubicada en Cantabria, España, es un destino ideal para disfrutar del sol, el mar y diversas actividades recreativas en un entorno natural espectacular.
    </p>

    <h2 class="section-header text-center">Entorno Natural</h2>
    <div class="info-box">
        <p class="section-content">La Playa de Laredo está rodeada de un entorno natural impresionante que incluye acantilados y áreas verdes protegidas. Las rutas de senderismo en los alrededores permiten explorar el paisaje costero y disfrutar de vistas panorámicas del mar. La playa está cerca del Parque Natural de las Marismas de Santoña, Victoria y Joyel, lo que añade un valor ecológico significativo y ofrece una experiencia enriquecedora para los amantes de la naturaleza y los visitantes en busca de belleza natural.</p>
    </div>

    <h2 class="section-header text-center">Gastronomía</h2>
    <div class="info-box">
        <p>Laredo es un destino excelente no solo para disfrutar del mar, sino también para los amantes de la buena gastronomía. En el paseo marítimo y en el centro del pueblo, encontrarás una amplia variedad de restaurantes y bares que ofrecen desde platos tradicionales cántabros hasta opciones más modernas. No te pierdas los pescados y mariscos frescos, así como los pintxos, que son una especialidad local.</p>
    </div>

    <h2 class="section-header text-center">Características</h2>
    <div class="info-box">
        <ul>
            <li>Arena fina y dorada</li>
            <li>Longitud: 4 km</li>
            <li>Mar tranquilo, ideal para familias</li>
            <li>Paseo marítimo amplio</li>
            <li>Servicios: duchas, aseos, socorristas</li>
            <li>Accesible para personas con movilidad reducida</li>
        </ul>
    </div>

    <h2>Datos de Interés</h2>
    <p>
        La Salvé es perfecta para familias y surfistas, ofreciendo servicios completos y vistas impresionantes del Mar Cantábrico. Su ubicación en Laredo permite disfrutar de una jornada de ocio completa, con una amplia oferta de restaurantes, bares y tiendas en los alrededores.
    </p>
    <h2>Historia y Cultura</h2>
    <p>
        La Playa de Laredo no solo es un lugar de belleza natural, sino que también tiene una rica historia vinculada a la villa de Laredo, una de las localidades más antiguas y con más tradición marinera de Cantabria. Laredo fue uno de los puertos más importantes durante la Edad Media y jugó un papel crucial en el comercio marítimo. Hoy en día, la ciudad conserva su encanto histórico, con un casco antiguo que invita a perderse por sus calles empedradas.
    </p>


    <h2>Eventos y Festividades</h2>
    <ul>
        <li><strong>Batalla de Flores:</strong> Una de las festividades más famosas de Laredo, celebrada a finales de agosto. Se trata de un desfile de carrozas decoradas con flores naturales, que culmina con una fiesta en la playa.</li>
        <li><strong>Fiesta de la Virgen del Carmen:</strong> Esta celebración marítima, en honor a la patrona de los marineros, incluye procesiones y eventos religiosos en la playa.</li>
        <li><strong>Carrera de Caballos:</strong> En verano, se celebra una espectacular carrera de caballos en la misma playa, un evento que atrae a numerosos visitantes.</li>
    </ul>
    <h2>Consejos para los Visitantes</h2>
    <ul>
        <li><strong>Mejor Época para Visitar:</strong> El verano es la temporada más popular, pero la playa también es hermosa en primavera y otoño, cuando es menos concurrida.</li>
        <li><strong>Amanecer y Atardecer:</strong> No te pierdas la oportunidad de disfrutar del amanecer o el atardecer desde la playa, los colores del cielo reflejándose en el mar son impresionantes.</li>
        <li><strong>Protección Solar:</strong> Debido a la duración de la exposición al sol, es recomendable utilizar protector solar y llevar sombrilla.</li>
        <li><strong>Explora Laredo:</strong> Además de la playa, explora el casco antiguo de Laredo, conocido como "La Puebla Vieja", lleno de historia y encanto.</li>
    </ul>
</section>