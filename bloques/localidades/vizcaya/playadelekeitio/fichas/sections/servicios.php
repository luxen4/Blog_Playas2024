<!-- Servicios -->
<section id="servicios" class="content-section container my-5">
    <h2 class="section-header text-center">Servicios en la Playa</h2>
    <ul class="list-unstyled">
        <li><strong>Alquiler de Barcos y Kayaks:</strong> Disponibilidad de embarcaciones y kayaks para explorar la costa y la Isla de San Nicolás.</li>
        <li><strong>Chiringuitos y Restaurantes:</strong> Variedad de bares y restaurantes donde degustar mariscos frescos y cocina local.</li>
        <li><strong>Servicios de Seguridad:</strong> Presencia de socorristas y puntos de primeros auxilios durante la temporada alta.</li>
        <li><strong>Zona de Juegos Infantiles:</strong> Áreas dedicadas para el entretenimiento de los más pequeños.</li>
    </ul>
</section>