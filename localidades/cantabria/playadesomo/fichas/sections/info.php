<?php 
$id = 'id="encabezado"';
$localidad = 'Somo'; // Cambiado para reflejar la nueva localidad
$region = 'Cantabria'; // Puedes ajustar la región según sea necesario
$li_encabezado = '
<header ' . $id . ' style="margin: 2em;" class="">
    <h2 style="text-align: center;" class="mb-0">Playa de ' . $localidad . '</h2>
    <p style="text-align: center;">Tu destino perfecto en ' . $region . '</p>

    <p class="text-center">Descubre la Playa de Somo, un tesoro en la costa de Cantabria. 
        <br>Con su extensa franja de arena dorada y aguas tranquilas, es el lugar ideal para disfrutar del sol, practicar deportes acuáticos y relajarse en un entorno natural impresionante. 
        <br>El paseo marítimo está repleto de bares y restaurantes que ofrecen la exquisita gastronomía local, convirtiendo tu visita en una experiencia inolvidable. 
        <br>La Playa de Somo es el destino perfecto para quienes buscan disfrutar del mar, la gastronomía y la tranquilidad de la naturaleza.
    </p>
</header>
';
?>
<?php $li_encabezado2 = ' '; ?>
<?php $li_encabezado3 = ' '; ?>

<?php $li_descripcion_detallada = '
<h2 class="section-header">Información General</h2>
<div class="info-box">
    <p class="section-content">La Playa de Somo, en Cantabria, es un popular destino para locales y turistas, conocido por su extensa arena dorada, condiciones ideales para el surf y un ambiente relajado. Además de la playa, Somo ofrece un paseo marítimo con bares y restaurantes, y está cerca de áreas naturales y rutas de senderismo, lo que lo convierte en un lugar perfecto para disfrutar de actividades al aire libre.</p>
</div>

<h2 class="section-header">Entorno Natural</h2>
<div class="info-box">
    <p class="section-content">Rodeada de dunas y áreas protegidas, la Playa de Somo es ideal para el senderismo y la observación de la naturaleza. Su cercanía a la Bahía de Santander añade valor ecológico, creando un entorno tranquilo y bello.</p>
</div>
'
; ?>


<?php $li_descripcion_detallada2 = '
<h2 class="section-header">Gastronomía</h2>
<div class="info-box">
    <p class="section-content">Somo es un paraíso para los surfistas y los amantes de la buena comida, con restaurantes que ofrecen platos cántabros tradicionales, como pescados, mariscos y pintxos locales.</p>
</div>
';?>

<?php $li_descripcion_detallada3 = '
<h2 class="section-header">Eventos Destacados</h2>
<div class="events">
    <ul>
        <li class="my-4"><span class="highlight">Festival de Surf de Somo:</span> <br>En agosto, reúne surfistas de todos los niveles para competir y disfrutar de un ambiente festivo.</li>
        <li class="my-4"><span class="highlight">Fiesta de San Juan:</span> <br>En junio, con hogueras, música en vivo y fuegos artificiales en la playa.</li>
        <li class="my-4"><span class="highlight">Mercado de Productos Locales:</span> <br>Durante el verano, se celebra en el paseo marítimo, con productos locales y gastronomía.</li>
        <li class="my-4"><span class="highlight">Conciertos al Aire Libre:</span> <br>Actuaciones musicales en el paseo marítimo durante el verano.</li>
        <li class="my-4"><span class="highlight">Competición de Vela:</span> <br>En septiembre, con regatas en la Bahía de Santander y eventos paralelos en Somo.</li>
    </ul>
</div>
'; ?>

<?php $li_mapa_ubicacion = '
<p>La Playa de Somo está a unos 30 minutos de Santander y es accesible en coche, autobús o ferry. También es posible llegar a pie o en bicicleta.</p>
<ul>
    <li class="my-4"><strong>En coche:</strong> <br>A 30 minutos de Santander por la S-10, con zonas de aparcamiento cercanas.</li>
    <li class="my-4"><strong>En autobús:</strong> <br>Hay autobuses regulares desde Santander y otras localidades cercanas.</li>
    <li class="my-4"><strong>En ferry:</strong> <br>Desde el puerto de Santander, un ferry te lleva directamente a Somo.</li>
    <li class="my-4"><strong>A pie o en bicicleta:</strong> <br>Somo cuenta con rutas peatonales y carriles bici que facilitan el acceso.</li>
</ul>
<h3>Mapa de la Playa de Somo</h3>
<iframe
    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2920.954508765897!2d-3.7342151842536696!3d43.45587537912983!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0xd491701e8553aab%3A0x123456789abcd!2sPlaya%20de%20Somo!5e0!3m2!1ses!2ses!4v1693554912315!5m2!1ses!2ses"
    width="100%" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade">
</iframe>
'; ?>

<?php $li_actividades_recomendadas = '
    <div class="info-box">
        <ul>
            <li class="my-4"><span class="highlight">Surf:</span> <br>Ideal para principiantes y expertos, con varias escuelas disponibles.</li>
            <li class="my-4"><span class="highlight">Windsurf:</span> <br>Perfecto gracias a las condiciones de viento favorables.</li>
'; ?>

<?php $li_actividades_recomendadas2 = '
            <li class="my-4"><span class="highlight">Paddle Surf:</span> <br>Alquiler de tablas y clases para disfrutar del mar de forma relajada.</li>
            <li class="my-4"><span class="highlight">Senderismo:</span> <br>Rutas que ofrecen vistas espectaculares del litoral cántabro.</li>     
'; ?>


<?php $li_actividades_recomendadas3 = '
            <li class="my-4"><span class="highlight">Vela:</span> <br>Clases disponibles en la bahía de Santander para todos los niveles.</li>
            <li class="my-4"><span class="highlight">Bicicleta:</span> <br>Rutas ciclistas que bordean la costa para explorar el entorno.</li>
       '; ?>

<?php $li_actividades_recomendadas4 = '
            <li class="my-4"><span class="highlight">Gastronomía:</span> <br>Variedad de restaurantes que ofrecen mariscos frescos y pintxos locales.</li>
            <li class="my-4"><span class="highlight">Eventos:</span> <br>Competiciones de surf, conciertos y fiestas tradicionales en verano.</li>
            <li class="my-4"><span class="highlight">Entorno Natural:</span> <br>Dunes y áreas protegidas perfectas para explorar y disfrutar de vistas espectaculares.</li>
        </ul>
    </div>
'; ?>

<?php $li_mas_info = '
    <div class="info-box">
        <h2 class="section-header">Más Info...</h2>  
        <p>Para más información sobre la Playa de Somo, puedes contactarnos a través de los siguientes medios:</p>
        <ul>
            <li class="my-4"><strong>Teléfono:</strong> <br>+34 942 51 12 34</li>
            <li class="my-4"><strong>Email:</strong> <br>turismo@somo.es</li>
            <li class="my-4"><strong>Oficina de Turismo:</strong> <br>Avenida de la Playa, 22, 39140 Somo, Cantabria
                <a href="https://www.somo.es/turismo"><strong>Web</strong></a>
            </li>
        </ul>
    </div>
'; ?>


<?php $li_services = '
    <div class="info-box">
        <ul>
            <li class="my-4"><span class="highlight">Socorristas:</span> <br>La playa cuenta con un servicio de socorrismo durante la temporada alta para garantizar la seguridad de los bañistas.</li>
            <li class="my-4"><span class="highlight">Accesibilidad:</span> <br>Acceso adaptado para personas con movilidad reducida, incluyendo rampas y pasarelas de madera que facilitan el acceso a la arena y al agua.</li>
            <li class="my-4"><span class="highlight">Duchas y baños:</span> <br>Instalaciones de duchas y baños ubicadas a lo largo de la playa para mayor comodidad de los visitantes.</li>
            <li class="my-4"><span class="highlight">Aparcamiento:</span> <br>Amplias zonas de aparcamiento cercanas a la playa, con espacios reservados para personas con discapacidad y aparcamiento para bicicletas.</li>
            <li class="my-4"><span class="highlight">Alquiler de equipos:</span><br> Disponibilidad de alquiler de tablas de surf, sombrillas y hamacas, así como de otros equipos deportivos y de playa.</li>
            <li class="my-4"><span class="highlight">Chiringuitos:</span> <br>Varios chiringuitos a lo largo de la playa ofrecen una variedad de bebidas, snacks y comidas, perfectos para una pausa junto al mar.</li>
        </ul>
    </div>
'; ?>


<?php
$imagenes = [

    [
        'src' => './../img/playa-somo.jpg',
        'alt' => 'Playa de Somo, vista panorámica',
        'figcaption' => 'Playa de Somo, Cantabria. Fuente:',
        'href' => 'https://saposyprincesas.elmundo.es/actividades-ninos/cantabria/aire-libre/playas/playa-de-somo',
        'titulo_href' => 'saposyprincesas.elmundo.es'
    ],
    [
        'src' => './../img/surf-playa-Somo-Cantabria.jpg',
        'alt' => 'Surf en Playa de Somo" class="img-fluid',
        'figcaption' => 'Surf en la Playa de Somo, Cantabria. Fuente:',
        'href' => 'https://elpais.com/elviajero/2021/01/21/actualidad/1611229533_330658.html#',
        'titulo_href' => 'elpais.com'
    ],
    [
        'src' => './../img/atardecer-playa-somo-cantabria.jpg',
        'alt' => 'Atardecer en la Playa de Somo en Cantabria.',
        'figcaption' => 'Atardecer en la Playa de Somo.',
        'href' => 'https://misfotosdecantabria.blogspot.com/2019/09/atardecer-desde-somo.html',
        'titulo_href' => 'misfotosdecantabria.blogspot.com'
    ],
    [
        'src' => './../img/senderismo-playa-somo-cantabria.jpg',
        'alt' => 'Senderismo cerca de Playa de Somo en Cantabria.',
        'figcaption' => 'Senderismo cerca de la Playa de Somo.',
        'href' => 'https://www.cantabriarural.com/blog/de-senderismo-por-la-costa-de-somo-langre-y-galizano/',
        'titulo_href' => 'cantabriarural.com'
    ],
    [
        'src' => './../img/vista-aerea-playa-Somo-Cantabria.jpg',
        'alt' => 'Vista aérea de Playa de Somo',
        'figcaption' => 'Vista aérea de la Playa de Somo, Cantabria.',
        'href' => 'https://es.kampaoh.com/guias-de-viaje/es/cantabria/que-ver-y-hacer-en-somo-cantabria-que-playas-rutas-y-visitas-se-recomiendan-donde-alojarse-y-comer-platos-tipicos/',
        'titulo_href' => 'es.kampaoh.com'
    ],
    [
        'src' => './../img/familias-difrutando-playa-somo-cantabria.jpg',
        'alt' => 'Familias disfrutando en la Playa de Somo, Cantabria',
        'figcaption' => 'Familias disfrutando en la Playa de Somo, Cantabria. Fuente:',
        'href' => '',
        'titulo_href' => ''
    ]
]; ?>